...
import { defaultProfileSidebar, IMAGES } from '@/lib/images'
...
export default function ProfileSidebar() {
  ...
  const [profileImage, setProfileImage] = useState(defaultProfileSidebar)
  ...
  const loadProfileData = () => {
    ...
    if (savedImage) {
      ...
      setProfileImage(defaultProfileSidebar)
    }
  }
    ...
    <div className={styles.sidebar}>
      ...
      <nav className="space-y-3">
        {menuItems.map((item) => (
          ...
          >
            <span className={`${item.fontSize} font-['Kanit'] -tracking-[0.333px] text-[#656565]`}>
              {item.label}
            </span>
          </Link>
        ))}
